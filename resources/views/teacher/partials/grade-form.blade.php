    ...
    <div class="card shadow-sm">
        <div class="card-header bg-primary text-white">
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    <h4 class="mb-0">Input Nilai: {{ $course->name }}</h4>
                    ...
                    <table class="table table-hover table-bordered">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td width="15%">
                                    <input type="number" 
                                        name="scores[{{ $student->id }}][assignment]" 
                                        class="form-control form-control-sm grade-input" 
                                        min="0" max="100" 
                                        step="0.01"
                                        data-student="{{ $student->id }}"
                                        value="{{ $existingGrades[$student->id]['assignment'] ?? '' }}"
                                        required>
                                </td>
                                <td width="15%">
                                    <input type="number" 
                                        name="scores[{{ $student->id }}][mid_exam]" 
                                        class="form-control form-control-sm grade-input" 
                                        min="0" max="100" 
                                        step="0.01"
                                        data-student="{{ $student->id }}"
                                        value="{{ $existingGrades[$student->id]['mid_exam'] ?? '' }}"
                                        required>
                                </td>
                                <td width="15%">
                                    <input type="number" 
                                        name="scores[{{ $student->id }}][final_exam]" 
                                        class="form-control form-control-sm grade-input" 
                                        min="0" max="100" 
                                        step="0.01"
                                        data-student="{{ $student->id }}"
                                        value="{{ $existingGrades[$student->id]['final_exam'] ?? '' }}"
                                        required>
                                </td>
                                <td width="15%">
                                    <div class="final-grade" 
                                        id="final-grade-{{ $student->id }}" 
                                        data-student="{{ $student->id }}">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function calculateFinalGrade(studentId) {
            const assignment = parseFloat(document.querySelector(`input[name="scores[${studentId}][assignment]"]`).value) || 0;
            const midExam = parseFloat(document.querySelector(`input[name="scores[${studentId}][mid_exam]"]`).value) || 0;
            const finalExam = parseFloat(document.querySelector(`input[name="scores[${studentId}][final_exam]"]`).value) || 0;
            ...
            if (finalGrade >= 85) {
                ...
                gradeElement.classList.add('text-warning');
            } else {
